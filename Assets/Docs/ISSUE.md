...
- 当前身体绑定可支持待机和原地步行测试，但膝部护甲接缝、肩髋权重及大幅动作穿插仍需逐动作检查。
- 原披肩仍采用初版分区骨骼与位置权重，垂臂时轮廓偏硬。新增独立 `PF_Zoey_Cloth` Unity Cloth 对比版，肩领固定并配置身体胶囊碰撞；尚未接入正式角色。极端动作穿插、硬质挂件与模拟布料之间的相对位移、传送重置及性能仍需后续验证。
- 布料批处理 Play Mode 冒烟测试未出现模拟发散，但 Editor 退出时报告 Persistent / JobTempAlloc 分配残留。尚未隔离其来源，不能据此归因于 Cloth，也不能视作完成内存稳定性验证；正式接入前应比较禁用布料的基线和多次进入退出播放的结果。
- `Idle` / `Walk` 已通过循环和 Humanoid 播放验证，尚未接入玩家控制；步行动画速度需要与实际移动速度匹配，不能仅凭原地预览判断滑步已解决。
