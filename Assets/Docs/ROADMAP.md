# EndLink Roadmap

EndLink 是一个早期 3D 连携战斗 demo，目标参考类似《异度之刃》和《明日方舟：终末地》的小队协同战斗体验。当前阶段仍处在白模验证期，优先建立单人控制、镜头、状态机和战斗基础能力，再逐步扩展到连携与小队表现。

文档更新原则：不变动结构，实际开发与计划有冲突则调整文档表述。
开发规划：有新增就新增，做完了就去掉，表述简洁。

## 后续开发规划

### 1. 单人行动基底
内容：基本完成

### 2. 连携战斗基底
内容：基础建设完成，具体规则有待设计。

### 3. 基础的队伍管理
内容：基本完成

### 4. 敌人基础制作

计划内容：后续接入战斗执行和行为树；
敌人目标选择/死亡与目标失效，保留必要死亡表现；
至少一个正式敌人可以主动接近并攻击主控，敌人死亡后能从目标系统和助战流程中稳定移除。

### 5. 连携触发与四类 Action Base 版

目标是在敌人基底可用于稳定测试后，做出第一版可验证的连携规则闭环：主控、队友、敌人、标签、事件和 Action 数据都能参与一次完整的“触发连携 -> 执行反应 -> 观察结果”流程。

计划内容：
- 连携触发规则初版：基于 `CombatEventsBus`、`CombatTagContainer` 和 `CombatTagDefinition`，定义最小可用的触发条件，例如指定标签命中、标签组合、目标处于可连携窗口等。
- 连携反应规则初版：触发后能明确由谁响应、响应哪个目标、执行哪个 Action，并广播可观察看清事件，方便调试。
- 四类 Action Base 版：分别做出`Skill`、`LinkAttack`、`Ultimate` 的基础数据资产和最小执行路径，键位。

### 6. 战斗系统整体基础建设（剩余）

计划内容：
- 角色数值系统：建立轻量 `CharacterStats` / 数值定义基础，用于承载攻击、防御、暴击率、暴击伤害、后续升级加点和临时数值修正，生命结算仍由 `CharacterHealth` 负责。
- 伤害结算管线：建立 `DamageCalculator` 和统一伤害上下文，让 Hitbox 命中后先经过攻击者数值、受击者数值、动作倍率和暴击等规则计算，再交给 `CharacterHealth` 扣血。
- Combat Target 标准化：扩展目标接口/组件，统一表达目标是否可选、是否存活、目标点、锁定点、受击点和 Collider 表面距离，供锁定、队友 AI、Hitbox 和 UI 共用。
- 受击规则：建立轻量受击策略，定义是否可被打断、Hit 硬直、击退倍率、死亡后的目标失效和碰撞处理边界。
- Action 执行统一接口：抽象玩家、队友、敌人的动作执行能力，统一 `CanExecute`、`TryExecute`、冷却查询和目标传入方式，减少 UI、AI、连携系统直接依赖具体 Driver。

### 7. 战斗 UI 第一版

目标是在已有玩家、队友、敌人和战斗动作基础上，补齐最小可读的战斗界面。

计划内容：
- 主控战斗 UI：显示主控生命值、当前主动技能冷却、连携请求槽位和极限技能量预留位置。
- 队友战斗 UI：显示两个队友的主动技能冷却、连携请求槽位和基础状态提示。
- 目标 UI：显示当前自动软锁目标的基础信息，例如敌人血量、目标点状态和后续可扩展的弱点/标签提示。
- 冷却反馈：复用并扩展 `UICombatActionSlot`，让技能槽、连携槽和后续大招槽统一读取动作与冷却来源。
- 连携预备 UI：先做占位显示，后续等连携规则落地后接入可释放、高亮和窗口倒计时。

阶段完成标准：
- 不看 Console 也能判断主控和队友技能是否可用。
- 自动软锁到敌人后能在 UI 上看到目标基础状态。
- UI 结构可以继续承接后续连携窗口、大招能量和队友状态显示。
