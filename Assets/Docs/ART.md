...
当前阶段的目标是形成一套轻量、清晰、可执行的美术资源协作规则。
...
美术资源使用独立 `art` 分支提交，程序主开发仍以 `dev` 分支为准。`art` 分支进入 `dev` 前，需要先经过固定复用的整理分支 `art-integration`。
...
`art-integration` 是固定整理台，不是每次新建的长期分支。每次同步前先让它对齐最新 `dev`，再把 `art` 的资源变更带入整理。

当需要同步美术资源时，可以让 Agent 执行固定流程：

1. 检查 `dev...art` 的变更范围。
2. 如果变更超出 `Assets/Art`，先停止并报告，不直接合并。
3. 切到固定复用的 `art-integration` 分支，并让它对齐当前 `dev`。
...
第一阶段使用人工规范和轻量自动校验。`Assets/Art/_Incoming` 已接入 `AssetPostprocessor` 监听，资源导入或移动后会自动检查 Shader、材质、Prefab、FBX、贴图和临时命名问题。Prefab 中附带的项目或外部脚本只提示人工确认，不直接判定为错误；`Loop Time` 等具体动作使用设置不纳入通用资源健康检查。

通过 `EndLink > Validation > Asset Validator` 可以查看完整结果、定位资源，并切换扫描 `_Incoming` 或整个 `Assets/Art`。第一版校验器只报告问题，不自动修改资源；确定安全的自动修复规则后续再逐步增加。
...
1. 继续补充场景层级、Layer、碰撞矩阵和资源目录校验。
2. 对明确且无歧义的规则提供手动“一键修复”，不默认自动改资源。
3. 在提交前或 CI 中运行同一套校验规则。
