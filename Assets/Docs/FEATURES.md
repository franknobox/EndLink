...
| [战斗动作配置](Features/COMBAT.md#feature-combat-action) | 已完成第一版 | 使用 `CombatActionDefinition` 数据资产描述普通攻击、技能、连携攻击和大招的伤害、冷却、时序、Hitbox 和命中标签。 |
...
| 功能名 | 当前状态 | 内容说明 |
| --- | --- | --- |
| [连携触发与窗口](Features/PARTY.md#feature-party-link-context) | 已完成第一版 | 协议反应触发后为三人小队开启 4 秒共享连携窗口，记录反应目标并允许玩家从三个连携技中选择一个释放。 |
...
| [队友有限状态机](Features/ALLY.md#feature-ally-state-machine) | 已完成通用动作状态版 | 提供 Idle、Follow、Assist、Action、Hit、Dead 外层状态，Assist 处理自动助战，Action 承载主动技能等指令动作。 |
...
| 功能名 | 当前状态 | 内容说明 |
| --- | --- | --- |
| [敌人身份与生命目标](Features/ENEMIES.md#feature-enemy-identity-health) | 已完成第一版 | 提供正式敌人根身份、生命受击、死亡事件、目标有效性、战斗标签容器和基础调试显示。 |
| [敌人感知与大状态机](Features/ENEMIES.md#feature-enemy-state-sensor) | 已完成骨架版 | 提供 Idle、Alert、Combat、Hit、Dead 大状态，以及第一版距离发现、警觉累计和进入战斗逻辑。 |
...
| 功能名 | 当前状态 | 内容说明 |
| --- | --- | --- |
| [战斗数据编辑工具](Features/TOOLS.md#feature-combat-data-tool) | 已完成字段同步版 | 提供 Editor 窗口快捷创建和查看战斗动作、战斗标签、标签组合规则数据资产，并显示关键字段摘要。 |
